...
\begin{figure}[H]
 ...
 \node[draw=techblue, fill=techblue!5, very thick, rectangle, rounded corners, text width=3.2cm, align=center, right=of tasks, xshift=5mm] (notify) {Email Notify};
...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.45\textwidth]{artifacts/latest_metric_summary.png}
 \caption{Severe class imbalance and the latest metric snapshot; use PR-AUC as primary metric.}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[width=0.45\textwidth]{artifacts/latest_pr_curve.png}
 \caption{Latest precision-recall curve from the validated run; shows trade-offs across thresholds.}
\end{figure}
...
\midrule
\textbf{XGBoost + KMeans (Current)} & \textbf{0.960122} & \textbf{0.775008} & \textbf{Winner}: Combines gradient boosting with unsupervised behavioral profiling. KMeans preserves high-level archetype structure while XGBoost learns micro-level feature splits. Maximizes both ranking quality and minority-class recovery. \\
\end{tabular}
\end{table}
...
ROC-AUC & 0.8870 & 0.960122 & Exceeded \\
...
\begin{figure}[H]
 ...
 \addplot+[fill=techblue!60] coordinates {(PR Target,0.8340) (PR Observed,0.775008) (ROC Target,0.8870) (ROC Observed,0.960122)};
 \end{axis}
 \end{tikzpicture}
