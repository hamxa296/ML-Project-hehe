...
We utilize a multi-stage Docker build to ensure environment parity. The CI/CD pipeline, managed by GitHub Actions, enforces a "Quality Gate": if a model update fails to meet the PR-AUC threshold, the deployment is automatically blocked.

\begin{figure}[H]
 \centering
 \resizebox{\columnwidth}{!}{%
 \begin{tikzpicture}[node distance=8mm and 8mm]
 \node[flowbox] (src) {Source Code};
 \node[flowbox, right=of src] (deps) {Install Dependencies};
 \node[flowbox, right=of deps] (image) {Build Docker Image};
 \node[flowbox, right=of image] (run) {Run Container};
 \node[flowbox, below=of image] (ship) {Ship to Runtime};

 \draw[flowarrow] (src) -- (deps);
 \draw[flowarrow] (deps) -- (image);
 \draw[flowarrow] (image) -- (run);
 \draw[flowarrow] (run) -- (ship);
 \end{tikzpicture}%
 }
 \caption{Multi-stage Docker build process for unified deployment.}
\end{figure}
...
\begin{figure}[H]
 ...
 \node[stage, right=of actions] (tests) {Lint and Tests};
 \node[stage, right=of tests] (build) {Build Artifact};
 ...
 \draw[flowarrow] (block.west) |- (tests.south);
 \end{tikzpicture}%
 }
 \caption{Automated CI/CD workflow from code push to deployment.}
\end{figure}
...
\subsection{What Each Tool Does}
\begin{table}[H]
\centering
\caption{Project components in clear interpretation}
\begin{tabular}{p{0.22\columnwidth}p{0.32\columnwidth}p{0.28\columnwidth}}
\toprule
Component & Simple role & When it matters \\
\midrule
Prefect & Runs tasks in order and records failures & During training and retraining \\
FastAPI & Serves predictions through HTTP & During inference and dashboard use \\
Docker & Packages the whole system the same way everywhere & During deployment and CI \\
Artifacts folder & Stores metrics and run history & After every successful run \\
\bottomrule
\end{tabular}
\end{table}
