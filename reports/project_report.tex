...
\begin{figure*}[t]
 \centering
 \includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/flowcharts/system_architecture.png}
 \caption{Complete system architecture showing CI/CD, orchestration, model registry, and serving layers with reproducible, script-generated layout.}
\end{figure*}
...
The dataset is heavily imbalanced, which is the fundamental challenge a reader must understand before interpreting any model score. Most rows are safe transactions, and a small minority are fraud. That means the model must be judged by whether it can find fraud, not by whether it can simply say "safe" all the time.
 ...
 \centering
 \begin{tabular}{cc}
  \includegraphics[width=0.46\textwidth]{../project/artifacts/report_figures/raw_eda/01_class_balance.png} &
  \includegraphics[width=0.46\textwidth]{../project/artifacts/report_figures/raw_eda/02_amount_distribution.png} \\
  \includegraphics[width=0.46\textwidth]{../project/artifacts/report_figures/raw_eda/04_missingness_correlation.png} &
  \includegraphics[width=0.46\textwidth]{../project/artifacts/report_figures/raw_eda/03_temporal_patterns.png}
 \end{tabular}
 \caption{Reproducible EDA outputs used in the report: class balance, transaction amount behavior, feature correlation structure, and temporal fraud patterns.}
\end{figure*}
...
The methodology follows a robust MLOps sequence: from raw ingestion and baseline comparison to advanced feature enrichment and ML health validation. Each stage has explicit quality criteria, and only stages that pass their quality gates proceed to the next phase.
 ...
 \begin{figure*}[t]
	\centering
	\includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/flowcharts/methodology_flow.png}
	\caption{End-to-end methodology flow from data ingestion through model validation and deployment, generated from scripts for clarity.}
\end{figure*}
...
\end{enumerate}
 ...
 \centering
 \includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/flowcharts/methodology_flow.png}
 \caption{End-to-end machine learning methodology from raw data to evaluation, generated programmatically for reproducibility.}
\end{figure*}
...
\begin{tikzpicture}[node distance=1cm and 1.2cm]
	...
	\node[flowbox, below=of time, yshift=-0.5cm] (log) {Log\\Transform};
	\node[flowbox, left=of log] (prune) {Drop\\>70\% Null};
	\node[flowbox, left=of prune] (freq) {Frequency\\Encode};
	\node[flowbox, left=of freq] (impute) {Median\\Impute};
  
	\node[flowbox, below=of impute, yshift=-0.5cm] (output) {Clean CSV\\(processed\_train.csv)};
...
\begin{tikzpicture}[node distance=0.8cm and 0.8cm]
	...
	\node[flowbox, below=of stage1c, yshift=-0.4cm] (stage1d) {Log\\Transform};
	\node[flowbox, left=of stage1d] (stage1e) {Prune\\Nulls};
	\node[flowbox, left=of stage1e] (stage1f) {Freq\\Encode};
	\node[flowbox, left=of stage1f] (stage1g) {Impute\\Median};
  
	\node[flowbox, below=of stage1g, yshift=-0.4cm] (clean) {Clean CSV\\(processed\_train.csv)};
	\node[flowbox, right=of clean] (split) {Stratified\\80/20\\Split};
	\node[flowbox, right=of split] (output) {\textbf{Train Set}\\(train\_unbalanced.csv)};
...
The project targets paper-style performance behavior. A simple benchmark view is shown below.
 ...
 \begin{figure*}[t]
	\centering
	\includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/model_eval/03_metric_summary.png}
	\caption{Paper benchmark context versus current run summary, rendered from reproducible evaluation artifacts.}
\end{figure*}
...
\begin{figure}[H]
 ...
 \draw[flowarrow] ([yshift=2mm]user.east) -- node[above, font=\scriptsize] {request} ([yshift=2mm]api.west);
 \draw[flowarrow] ([yshift=-2mm]api.west) -- node[below, font=\scriptsize] {response} ([yshift=-2mm]user.east);
 
 \draw[flowarrow] ([yshift=2mm]api.east) -- node[above, font=\scriptsize] {load/score} ([yshift=2mm]model.west);
 \draw[flowarrow] ([yshift=-2mm]model.west) -- node[below, font=\scriptsize] {probability} ([yshift=-2mm]api.east);
...
\begin{figure}[H]
 ...
 \node[stage, right=of frontend] (backend) {FastAPI Backend};
 \node[stage, below=of backend] (models) {Models and Artifacts};
 \node[stage, below=of frontend] (data) {Shared Data Volume};

 \draw[flowarrow] ([yshift=2mm]frontend.east) -- node[above, font=\scriptsize] {API calls} ([yshift=2mm]backend.west);
 \draw[flowarrow] ([yshift=-2mm]backend.west) -- node[below, font=\scriptsize] {responses} ([yshift=-2mm]frontend.east);
 
 \draw[flowarrow] ([xshift=2mm]backend.south) -- node[right, font=\scriptsize] {load / save} ([xshift=2mm]models.north);
 \draw[flowarrow] ([xshift=-2mm]models.north) -- node[left, font=\scriptsize] {predictions} ([xshift=-2mm]backend.south);
 
 \draw[flowarrow] (backend.south west) -- node[sloped, above, font=\scriptsize] {read features} (data.north east);
 \end{tikzpicture}%
 }
...
\begin{figure}[H]
 ...
 \node[stage, right=of gate, xshift=4mm] (deploy) {Deploy};
 \node[stage, below=of gate, yshift=-4mm] (block) {Block\\Release};

 \draw[flowarrow] (push) -- (actions);
 \draw[flowarrow] (actions) -- (tests);
 \draw[flowarrow] (tests) -- (build);
 \draw[flowarrow] (build) -- (gate);
 \draw[flowarrow] (gate.east) -- node[above, font=\scriptsize] {pass} (deploy.west);
 \draw[flowarrow] (gate.south) -- node[right, font=\scriptsize] {fail} (block.north);
 \end{tikzpicture}%
 }
...
All machine learning tasks are orchestrated by Prefect. This ensures that data loading, training, and evaluation occur in a strict, observable sequence with automated retries and failure logging.
 ...
 \begin{figure*}[t]
  \centering
 \includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/flowcharts/prefect_orchestration.png}
 \caption{Prefect orchestration flow for model training and validation, generated programmatically.}
\end{figure*}
...
A full run starts by loading the CSV files, then the pipeline performs EDA on the raw data, trains the model, runs EDA again on the transformed data, evaluates the result, writes metrics to disk, and only then saves the model. That order is deliberate: it gives us visibility into both the starting data and the transformed features.
 ...
 \begin{figure*}[t]
  \centering
 \includegraphics[width=0.95\textwidth]{../project/artifacts/report_figures/flowcharts/containerization_workflow.png}
 \caption{The end-to-end run order used by Prefect and the training pipeline, shown with reproducible orchestration graphics.}
\end{figure*}
