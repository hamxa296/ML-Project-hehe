...
\begin{figure}[H]
 ...
 \node[flowbox, below=of data] (clean) {Clean};
 \node[flowbox, below=of clean] (learn) {Learn};
 \node[flowbox, below=of learn] (score) {Score};
 \node[flowbox, below=of score] (serve) {Serve};
...
\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/03_metric_summary.png}
 \caption{Metric summary from the production run against paper benchmark targets. ROC-AUC exceeds the benchmark; PR-AUC is near-target, confirming the model is competitive for real-world fraud retrieval.}
\end{figure}
