...
\title{\huge \textbf{Architecting a Production-Grade Fraud Detection Pipeline}}
\author{%
...
The dataset is heavily imbalanced. The class distribution plot below shows the 27.6:1 safe-to-fraud ratio that drives every design choice in this project — from choosing PR-AUC as the primary metric to using \texttt{scale\_pos\_weight} in XGBoost.

\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/01_class_balance.png}
 \caption{Class balance: 96.5\% safe vs 3.5\% fraud. The 27.6:1 ratio makes accuracy useless as a metric — a model that predicts "safe" always achieves 96.5\% accuracy while catching zero fraud.}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/02_amount_distribution.png}
 \caption{Transaction amount distributions by class. Fraud exhibits an 18\% higher standard deviation with bimodal behavior: micro-transactions (card testing $<$\$1) and large outlier drains. Log-scale applied to compress the right tail.}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/03_temporal_patterns.png}
 \caption{Temporal fraud patterns by hour and day. Elevated fraud rates during 00:00--06:00 UTC indicate fraudsters exploit low-monitoring windows, directly motivating the extraction of \texttt{hour} and \texttt{day\_of\_week} features.}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/04_missingness_correlation.png}
 \caption{Missingness heatmap across feature families. Over 91\% of columns exceed 70\% missingness (V-feature families are especially sparse), validating the aggressive null-pruning strategy applied in Stage 1.}
\end{figure}

\begin{table}[H]
\centering
\caption{What the raw dataset is telling us}
\begin{tabular}{p{0.31\columnwidth}p{0.58\columnwidth}}
\toprule
\textbf{What we see} & \textbf{What it means} \\
\midrule
...
\toprule
\textbf{Step} & \textbf{What it does} & \textbf{Why it helps} \\
\midrule
...
\begin{figure}[H]
 ...
 \node[flowbox, below=of raw] (prune) {Feature Pruning};
 \node[flowbox, below=of prune] (engineer) {Feature Engineering};
 \node[flowbox, below=of engineer] (cluster) {K-Means Clustering};
 \node[flowbox, below=of cluster] (ensemble) {Stacked Ensemble};
 \node[flowbox, below=of ensemble] (eval) {Model Evaluation};
...
\toprule
\textbf{Model} & \textbf{{ROC-AUC}} & \textbf{{PR-AUC}} & \textbf{\multicolumn{1}{p{0.36\columnwidth}}{Theoretical backing and why chosen or rejected}} \\
\midrule
...
\toprule
\textbf{Metric} & \textbf{Interpretation} & \textbf{Desired behavior} \\
\midrule
...
\toprule
\textbf{Metric / Reference} & \textbf{{Target}} & \textbf{{Observed}} & \textbf{Status} \\
\midrule
...
\toprule
\textbf{Operating Regime} & \textbf{{Precision}} & \textbf{{Recall}} & \textbf{{F1}} \\
\midrule
...
\begin{figure}[H]
 ...
 \caption{Validated ROC and PR curves from the production run. The ROC-AUC of 0.965 (exceeds the 0.887 benchmark) and PR-AUC of 0.793 (near the 0.834 target) confirm the model's strong ranking quality under heavy class imbalance.}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[width=\columnwidth]{assets/04_confusion_matrix.png}
 \caption{Confusion matrix from the production model at the balanced operating threshold. High recall ensures most fraud is captured; false positives remain manageable within operational review capacity.}
\end{figure}
...
\toprule
\textbf{Option} & \textbf{Strength} & \textbf{Limitation on this dataset} \\
\midrule
...
\begin{figure}[H]
 ...
 \caption{Precision-recall threshold trade-off from actual model evaluation. Raising the decision threshold improves precision but reduces recall — the operating regime is selected based on the business cost of missed fraud vs. false alarms.}
\end{figure}
...
\toprule
\textbf{Goal} & \textbf{Threshold style} & \textbf{Business effect} \\
\midrule
...
\toprule
\textbf{Parameter} & \textbf{Value/Range} & \textbf{Theoretical and Empirical Justification} \\
\midrule
...
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Operational justification} \\
\midrule
